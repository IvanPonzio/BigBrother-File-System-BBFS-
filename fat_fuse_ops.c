...
static void fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    fat_file log_file = fat_tree_get_file(file_node);
    fat_file parent = fat_tree_get_parent(file_node);
    off_t offset = log_file->dentry->file_size;
    fat_file_pwrite(log_file,buf,message_size,offset,parent);
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    /* PRIMERA PARTE */
    if(strcmp(ROOT_DIRNAME, dir->name) == 0){
        
        u32 bb_cluster = search_bb_orphan_dir_cluster();
        
        if(bb_cluster > 0){
            bb_init_log_dir(bb_cluster);
            fat_tree_node bb_node = fat_tree_node_search(vol->file_tree, "/bb");;
            fat_file bb_dir = fat_tree_get_file(bb_node);
            GList *fs_log_ptr = fat_file_read_children(bb_dir);
            fat_tree_insert(vol->file_tree,bb_node,(fat_file)fs_log_ptr->data);


        }else {
            
            u32 bb_data_cluster = fat_table_get_next_free_cluster(vol->table);
            //check errors 
            
            fat_table_set_next_cluster(vol->table, bb_data_cluster, FAT_CLUSTER_BAD_SECTOR);
            bb_init_log_dir(bb_data_cluster);
            bb_fat_fuse_mknod(BB_LOG_FILE, 0, 0);
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        if(strcmp((*child)->name, "bb") && strcmp((*child)->name, "fs.log") ){
...
int fat_fuse_unlink(const char *path){
    ...
    parent = fat_tree_get_parent(file_node);
    //free al @file clusters
    DEBUG("Starting to free file clusters..");
    fat_file_free_clusters(file);
    
    //remove @file dentry
    DEBUG("Starting to remove file dentry..");
    fat_file_remove_dentry(file, parent);

    //delete @file node from tree
    DEBUG("Starting to remove tree node..");
...
int fat_fuse_rmdir(const char *path){
    ...
    //print_array(fat_tree_flatten_h_children(file_node));
    if(fat_tree_flatten_h_children(file_node)[0] == NULL){
        parent_node = fat_tree_node_search(vol->file_tree, dirname(strdup(path)));
        if (parent_node == NULL) {
            errno = ENOENT;
            return -errno;
        }
        fat_file parent = fat_tree_get_file(parent_node);
        fat_file_free_clusters(directory);
        fat_file_remove_dentry(directory, parent); 
        vol->file_tree = fat_tree_delete(vol->file_tree, path);
    }else{
        fprintf(stdout, "Directory is not empty\n");
    }
    return EXIT_SUCCESS;
    
}

int bb_fat_fuse_mknod(const char *path, mode_t mode, dev_t dev) {
    errno = 0;
    fat_volume vol;
    fat_file parent, new_file;
    fat_tree_node parent_node;
...
// void print_array(fat_file *array){
//     int i = 0; 
//     do{
//         DEBUG("elemento del array: %p", array[i]);
//         i++;
