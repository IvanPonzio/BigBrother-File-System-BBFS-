...
void fat_file_free_clusters(fat_file file){
    u32 next_cluster;
...
void fat_file_remove_dentry(fat_file file, fat_file parent){
    //Write first byte 0xE5 (marked as pending for deletion)
    DEBUG("Writing 0xE5 to dentry\n");
    file->dentry->base_name[0] = FILE_FOR_DELETION;
    //parent->dir.nentries--;
    //DEBUG("Decrementing parent nentries\n");
    write_dir_entry(parent, file);
    DEBUG("Writing dentry to disk finished");
}


// //read bb_dir entry from disk's @cluster and returns the file

// fat_file bb_read_dir_entry(fat_file root_dir, u32 cluster){
//     u8 *buf = NULL;
//     size_t length = sizeof(struct fat_dir_entry_s);
//     buf = alloca(length);
//     fat_dir_entry disk_dentry_ptr;
//     off_t cluster_offset = fat_table_cluster_offset(root_dir->table, cluster);

//     if (full_pread(root_dir->table->fd, buf, length, cluster_offset ) !=
//         length) {
//         errno = EIO;
//         DEBUG("Fail to read BB HIDDEN CLUSTER\n");
     
//         return NULL;
//     }
//     disk_dentry_ptr = (fat_dir_entry)buf;
//     fat_dir_entry bb_entry = init_direntry_from_buff(disk_dentry_ptr);
//     fat_file bb_dir = init_file_from_dentry(bb_entry, root_dir);
//     return bb_dir;
// }
